    ...
    <table class="w-full mt-2 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            @forelse ($brands as $brand)
                <tr class="{{ $brand->is_active == 1 ? 'text-green-700' : 'text-gray-300' }} border-b text-xs leading-8 odd:bg-white even:bg-gray-50">
                    <th scope="row" class="px-2 py-1">
                        <span>{{ $loop->iteration }}]</span>
                    </th>
                    <td class="py-1 {{ $brand->is_active == 0 ? 'opacity-15' : '' }}">
                        <img src="{{ asset("storage/{$brand->image}") }}" class="w-12 py-1 px-1 shadow" alt="&#128228;">
                    </td>
                    <td class="px-2 py-1">
                    ...
                    <td class="px-2">
                        ...
                        <div class="flex gap-1">
                            @if ($brand->is_active == 1)
                                <a href="{{ route('brandsedit', $brand->id) }}">
                                ...
                                <a href="{{ route('brandsactive', $brand->id) }}" onclick="return confirm('Tem Certeza que Deseja (DESATIVAR)?')">
                            ...
                            @else
                                ...
                                <a href="{{ route('brandsactive', $brand->id) }}" onclick="return confirm('Tem Certeza que Deseja (REATIVAR)?')">
